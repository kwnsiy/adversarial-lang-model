...
from lm.const_generator_gru import Generator

# you can use this as a dialogue act classifier
from lm.const_discriminator import Discriminator
from lm.normal_discriminator import Discriminator as DAClassifer


from lm.const_rollout import Rollout
...
da_model_path   = "null"
disc_pretrain = False
disc_model_path = "null"


#pretrain = False
#model_path = "./lm/out/mle_epochs/const_lm_gru,5,epoch,11.314596176147461,32.38923645019531,.model"
#a_pretrain = False
#da_model_path = "./lm/out/mle_epochs/da_classifier,0,epoch,27.878992080688477,33.82069778442383,0.8365729754896072,0.805417508984726,.model"
#disc_pretrain = False
#disc_model_path = "./lm/out/mle_epochs/const_lm_disc_pre,40.666648864746094,0.6357291228439209,41.27981185913086,0.6609466221851543,41.672637939453125,0.645959051724138,.model"
...
NEGATIVE_FILE = ['./lm/out/gen/const_lm_gru_fake_src_words.txt' , './lm/out/gen/const_lm_gru_fake_tgt_words.txt' , './lm/out/gen/const_lm_gru_fake_tgt_da_id.txt']
TEMP_FILE     = ['./lm/out/gen/const_lm_gru_temp_src_words.txt' , './lm/out/gen/const_lm_gru_temp_tgt_words.txt' , './lm/out/gen/const_lm_gru_temp_tgt_da_id.txt']
...
def train_disc(model, data_iter, criterion, optimizer):
    ...
    for (src, tgt, da, rf, drf) in tqdm(data_iter, mininterval=2, desc=' - Training', leave=False):
        drf = rf
        src, tgt, da, rf, drf  = Variable(src), Variable(tgt), Variable(da), Variable(rf), Variable(drf)
        if opt.cuda: src, tgt, da, rf, drf = src.cuda(), tgt.cuda(), da.cuda(), rf.cuda(), drf.cuda()
        optimizer.zero_grad()
        pred = model.forward(tgt, da)
...
def train_da(model, data_iter, criterion, optimizer):
    data_iter.reset()
    total_loss, total_words, pred, ite, acc = 0., 0., 0., 0., 0
    for (src, tgt, da, rf, drf) in tqdm(data_iter, mininterval=2, desc=' - Training', leave=False):
        src, tgt, da, rf, drf  = Variable(src), Variable(tgt), Variable(da[:,1]), Variable(rf), Variable(drf)
        if opt.cuda: src, tgt, da, rf, drf = src.cuda(), tgt.cuda(), da.cuda(), rf.cuda(), drf.cuda()
        optimizer.zero_grad()
        pred = model.forward(tgt)
        loss = criterion(pred, da)
        total_loss += loss.data[0]
        loss.backward()
        optimizer.step()
        ite += 1
        _, predicted = torch.max(pred.data, 1)
        acc += (predicted == da).sum().item() / da.size(0)
    print (acc/ ite)
    return total_loss / ite, acc/ ite

# eval epoch
def eval_da(model, data_iter, criterion):
    data_iter.reset()
    total_loss, total_words, pred, ite, acc = 0., 0., 0., 0., 0
    trues, predicts = [], []
    for (src, tgt, da, rf, drf) in tqdm(data_iter, mininterval=2, desc=' - Training', leave=False):
        ...
        _, predicted = torch.max(pred.data, 1)
        acc += (predicted == da).sum().item() / da.size(0)
        trues.extend(da.tolist())
        predicts.extend(predicted.tolist())
    print (acc/ ite)
    print (confusion_matrix(trues, predicts))
...
def main():
    ...
    # Define Networks
    generator = Generator(TGT_VOCAB_SIZE, 4, g_emb_dim, g_hidden_dim, opt.cuda, start_id, padding_id)
    discriminator = Discriminator(d_num_class, 4, TGT_VOCAB_SIZE, d_emb_dim, d_filter_sizes, d_num_filters, d_dropout, padding_id)
    classifier    = DAClassifer(d_num_class, 4, TGT_VOCAB_SIZE, d_emb_dim, d_filter_sizes, d_num_filters, d_dropout, padding_id)        
    ...
    if pretrain:
        ...
        for epoch in range(PRE_EPOCH_NUM):
            ...
            torch.save(generator.state_dict(), "./lm/out/mle_epochs/const_lm_gru," + str(epoch) + ",epoch," + 
    ...
    # Pretrain Classifier
    da_criterion = nn.NLLLoss(size_average=False).cuda()
    da_optimizer = optim.SGD(classifier.parameters(), lr=1e-3)
    ...
    if da_pretrain:
        for epoch in range(1):
            ...
            da_data_iter   = DisDataIter(POSITIVE_FILE, TEMP_FILE, BATCH_SIZE, VOCAB_MODEL, g_sequence_len)
            eval_data_iter = DisDataIter(EVAL_FILE, TEMP_FILE, BATCH_SIZE, VOCAB_MODEL, g_sequence_len)
            for _ in range(30):
                train_loss, da_acc = train_da(classifier, da_data_iter, da_criterion, da_optimizer)
                print('Epoch [%d], loss, acc: %f, %f' % (epoch, train_loss, da_acc))
                eval_loss, eval_acc = eval_da(classifier, eval_data_iter, da_criterion)
                print('Epoch [%d], True loss, acc: %f, %f' % (epoch, eval_loss, eval_acc))
                torch.save(classifier.state_dict(), "./lm/out/mle_epochs/da_classifier," + str(epoch) + ",epoch," + 
                    str(float(train_loss)) + "," + str(float(eval_loss)) + ","  +
                    str(float(da_acc)) + "," + str(float(eval_acc)) + ",.model")
    else: 
        classifier.load_state_dict(torch.load(da_model_path))
        classifier = classifier.cuda()
    ...
    if disc_pretrain:
        ...
        torch.save(discriminator.state_dict(), "./lm/out/mle_epochs/const_lm_disc_pre," +
    ...
    for total_batch in range(TOTAL_BATCH):
        ...
        if total_batch % 1 == 0 or total_batch == TOTAL_BATCH - 1:                
            ...
            torch.save(generator.state_dict(), "./lm/out/gan_epochs/const_lm," + str(total_batch) + 
        ...
        if total_batch % 1 == 0 or total_batch == TOTAL_BATCH - 1: 
            ...
            eval_loss, eval_acc = eval_disc(discriminator, dis_data_iter, dis_criterion)
            # eval pretrain discriminators
            torch.save(discriminator.state_dict(), "./lm/out/gan_epochs/const_lm_disc," + str(total_batch) + ",batch," + 
                str(float(disc_loss))  + ","  + str(float(disc_acc)) + "," +  
                str(float(train_loss)) + ","  + str(float(train_acc)) + "," + 
                str(float(eval_loss))  + ","  + str(float(eval_acc))  + "," + ".model")
    
        if total_batch % 1 == 0 or total_batch == TOTAL_BATCH - 1: 
            eval_data_iter = DisDataIter(NEGATIVE_FILE, TEMP_FILE, BATCH_SIZE * 10, VOCAB_MODEL, g_sequence_len)
            eval_loss, eval_acc = eval_da(classifier, eval_data_iter, da_criterion)
            print('Epoch [%d], Generated classification loss, acc: %f, %f' % (total_batch, eval_loss, eval_acc))
